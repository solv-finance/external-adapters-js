...
export interface AdapterContext {
  name?: string
  cache?: import('./lib/cache/types').CacheImplOptions
  rateLimit?: import('./lib/rate-limit/config').Config
}
...
export class BigNumber implements Hexable {
  ...
  constructor(constructorGuard: unknown, hex: string)
  ...
  static from(value: unknown): BigNumber
  static isBigNumber(value: unknown): value is BigNumber
}
export type Value = BigNumberish | BigNumberish[]
// Allow 3 levels of nesting
export type NestableValue =
  | AdapterValue
  | Record<string, AdapterValue>
  | Record<string, Record<string, AdapterValue>>
...
export interface AdapterRequestData extends AdapterData {
  ...
  includes?: {
    from: string
    to: string
    includes: [{ from: string; to: string; adapters: string[]; inverse: boolean; tokens: boolean }]
  }[]
  maxAge?: string
  cost?: number
}
...
export type AdapterResponse = {
  ...
  result: AdapterValue // Result for OCR
  maxAge?: number
  metricsMeta?: AdapterMetricsMeta
  debug?: AdapterDebug
...
export interface APIEndpoint<C extends Config = Config> {
  ...
  batchablePropertyPath?: import('./lib/cache-warmer/reducer').BatchableProperty[]
...
export interface WSHandler {
  ...
  heartbeatMessage?: (id: number, connectionParams: Record<string, unknown>) => UnknownWSMessage
